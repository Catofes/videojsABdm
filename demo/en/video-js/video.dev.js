...
vjs.Player.binddm = function(playerUnit,video){
	if(typeof CommentManager !== "undefined"){
		vjs.cmManager = new CommentManager(vjs.divComment);
		vjs.cmManager.display = true;
		vjs.cmManager.init();
		vjs.cmManager.clear();
		var lastPosition = 0;
		if(vjs.state.autosize){
		...
		video.addEventListener("timeupdate", function(){
			if(vjs.cmManager.display === false) return;
			if(video.hasStalled){
				vjs.cmManager.startTimer();
				video.hasStalled = false;
			}
			vjs.cmManager.time(Math.floor(video.currentTime * 1000));
		});
		...
		video.addEventListener("ratechange", function(){
			if(vjs.cmManager.def.globalScale != null){
				if(video.playbackRate !== 0){
					vjs.cmManager.def.globalScale = (1 / video.playbackRate);
					vjs.cmManager.rescale();
				}
			}
		});
